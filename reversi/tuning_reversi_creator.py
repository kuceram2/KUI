# import random_player
import getopt
import sys
import time

from inspect import signature
from game_board import GameBoard
#from player_creator import create_player

DEBUG = False
MAX_TIME_FOR_MOVE = 5

class HeadlessReversiCreator(object):
    """
    Creator of the Reversi game without the GUI.
    """

    def __init__(self, player1, player1_color, player2, player2_color, board_size=8):
        """
        :param player1: Instance of first player
        :param player1_color: color of player1
        :param player2: Instance of second player
        :param player1_color: color of player2
        :param board_size: Board will have size [boardSize x boardSize]
        """
        self.board = GameBoard(board_size, player1_color, player2_color)
        self.player1 = player1
        self.player2 = player2
        self.current_player = self.player1
        self.current_player_color = player1_color
        self.player1_color = player1_color
        self.player2_color = player2_color

    def play_game(self):
        """
        This function contains game loop that plays the game.
        """
        # TODO: Time limit for move - because fuck the students amirite?!
        correct_finish = True
        while self.board.can_play(self.current_player_color):
            startTime = time.time()
            move = self.current_player.select_move(self.board.get_board_copy())
            endTime = time.time()
            moveTime = (endTime - startTime) * 1000
            if moveTime > MAX_TIME_FOR_MOVE * 1000:
                #print("Player %d took too long: %.3f ms." % (self.current_player_color, moveTime))
                if(self.current_player_color == 0): sys.exit(90)
                else: sys.exit(91)

            if move is None:
                if(DEBUG): print(
                    "Player %d reurns None instead of a valid move. Move takes %.3f ms."
                    % (self.current_player_color, moveTime)
                )
                correct_finish = False
                break
            else:
                if(DEBUG): print(
                    "Player %d wants move [%d,%d]. Move takes %.3f ms."
                    % (self.current_player_color, move[0], move[1], moveTime)
                )

            move = (int(move[0]), int(move[1]))
            if self.board.is_correct_move(move, self.current_player_color):
                if(DEBUG): print("Move is correct")
                self.board.play_move(move, self.current_player_color)

            else:
                if(DEBUG): print(
                    "Player %d made the wrong move [%d,%d]"
                    % (self.current_player_color, move[0], move[1])
                )
                correct_finish = False
                break

            self.change_player()
            if not self.board.can_play(self.current_player_color):
                if(DEBUG): print("No possible move for Player %d" % (self.current_player_color))
                self.change_player()
                if self.board.can_play(self.current_player_color):
                    if(DEBUG): print("Player %d plays again " % (self.current_player_color))
                else:
                    if(DEBUG):print("Game over")

            if(DEBUG): self.board.print_board()
        if correct_finish:
            self.printFinalScore()
        else:
            if(DEBUG):print("Game over.")
            if self.current_player_color == self.player1_color:
                if(DEBUG):print("Winner is player %d." % (self.player2_color))
            else:
                if(DEBUG):print("Winner is player %d." % (self.player1_color))

    def change_player(self):
        """
        Change the current_player
        """
        if self.current_player == self.player1:
            self.current_player = self.player2
            self.current_player_color = self.player2_color
        else:
            self.current_player = self.player1
            self.current_player_color = self.player1_color

    def printFinalScore(self):
        p1Stones = 0
        p2Stones = 0
        for x in range(self.board.board_size):
            for y in range(self.board.board_size):
                if self.board.board[x][y] == 0:
                    p1Stones += 1
                if self.board.board[x][y] == 1:
                    p2Stones += 1

        if(DEBUG):print("\n\n-----------------------------\n")
        if(DEBUG):print(
            "Final score:\n\nPlayer%d:Player%d\n\t[%d:%d]\n"
            % (self.player1_color, self.player2_color, p1Stones, p2Stones)
        )
        if p1Stones > p2Stones:
            if(DEBUG):print("Player %d wins!" % (self.player1_color))
            sys.exit(100+self.player1_color)
        elif p2Stones > p1Stones:
            if(DEBUG):print("Player %d wins!" % (self.player2_color))
            sys.exit(100+self.player2_color)
        else:
            if(DEBUG):print("Draw")
            sys.exit(99)
        if(DEBUG):print("\n-----------------------------\n\n")

   

def create_player(player_class, my_color, opp_color, param_idx, param_val, board_size):
    num_params = len(signature(player_class).parameters)
    if num_params == 2:
        player = player_class(my_color, opp_color)
    elif num_params == 3:
        player = player_class(my_color, opp_color, board_size)
    elif num_params == 5:
        if(DEBUG):print("creating in tuning mode")
        player = player_class(my_color, opp_color, param_idx, param_val, board_size)
    else:
        raise NotImplementedError
    return player

def main():
    (choices, args) = getopt.getopt(sys.argv[1:], "")
    # print(choices)
    # print(args)
    # print("**************/*/*/*/*/*/**/*//*/**//*/")
    p1_color = 0
    p2_color = 1
    board_size = 8

    importsCorrect = True
    colors = [p1_color, p2_color]
    players = []
    param_idx = args[1]
    param_val = args[2]
    # if only one player specified, it will play againts itself
    for i in [0, -1]:
        try:
            to_import = args[i]
            if ".py" in args[i]:
                to_import = args[i].replace(".py", "")
            #print("importing", to_import)
            player_module = __import__(to_import)
            players.append(
                create_player(
                    player_module.MyPlayer, colors[i], colors[i + 1], param_idx, param_val, board_size
                )
            )
        except ImportError:
            importsCorrect = False
            print("Error: Cannot import given player: %s." % (args[i]))

    if importsCorrect:
        game = HeadlessReversiCreator(
            players[0], p1_color, players[1], p2_color, board_size
        )
        game.play_game()
    
    sys.exit(69)

if __name__ == "__main__":
    main()